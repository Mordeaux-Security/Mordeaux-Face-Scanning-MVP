...
document.getElementById("search-form").addEventListener("submit", async (event) => {
  ...
  try {
    ...
    const payload = {
      ...
      image_b64: imageB64,
      top_k: 10,
      threshold: 0.5,  // Lower threshold to show more results
    };
    ...
    searchStatus.innerHTML = `<span style="color:#28a745;">✅ Found ${json.count} match${json.count > 1 ? 'es' : ''}!</span>
      <span style="color:#666; font-size:13px;"> (showing faces with ≥${Math.round(payload.threshold * 100)}% similarity)</span>`;
    
    renderSearchResults(json.hits);
  } catch (err) {

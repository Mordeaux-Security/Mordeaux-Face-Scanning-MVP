...
class HybridCacheService:
    ...
    async def get_cache_stats(self) -> Dict[str, Any]:
        """Get comprehensive cache statistics from both Redis and PostgreSQL."""
        stats = {
        ...
        if self.postgres_enabled:
            try:
                loop = asyncio.get_event_loop()
                postgres_stats = await loop.run_in_executor(
                    self._thread_pool,
                    self._get_postgres_stats
                )
                stats['postgres_stats'] = postgres_stats
            except Exception as e:
                stats['postgres_stats'] = {'error': str(e)}

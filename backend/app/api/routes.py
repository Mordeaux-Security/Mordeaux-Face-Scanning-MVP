...
def _require_image(file: UploadFile, content: bytes):
    if not content:
        raise create_http_exception("EMPTY_FILE")
    
    ctype = (file.content_type or "").lower()
    if ctype not in {"image/jpeg", "image/jpg", "image/png"}:
        raise create_http_exception("INVALID_IMAGE_FORMAT")
    
    # Check file size (10MB limit)
    if len(content) > 10 * 1024 * 1024:
        raise create_http_exception("IMAGE_TOO_LARGE")
            ...
            "content": {
                "application/json": {
                    "example": {
                        ...
                        "thumb_url": "/api/images/thumbnails/tenant123/abc123_thumb.jpg",
                        ...
                        "results": [
                            {
                                ...
                                "score": 0.95,
                                "metadata": {
                                    "raw_url": "/api/images/raw-images/tenant123/def456.jpg",
                                    "thumb_url": "/api/images/thumbnails/tenant123/def456_thumb.jpg",
...
async def search_face(request: Request, file: UploadFile = File(...), top_k: int = None, threshold: float = None):
...
async def compare_face(request: Request, file: UploadFile = File(...), top_k: int = None, threshold: float = None):
    """
...
async def create_batch_index_job(request: Request, batch_request: BatchIndexRequest, background_tasks: BackgroundTasks):
...
async def register_webhook(request: Request, webhook_request: WebhookRegistrationRequest):
    ...
    # Validate events
    valid_events = ["face.indexed", "face.searched", "face.compared", "batch.created", "batch.completed", "batch.failed"]

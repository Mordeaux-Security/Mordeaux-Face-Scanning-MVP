...
class Settings(BaseSettings):
    ...
    rate_limit_requests_per_second: float = 10.0  # Sustained rate
    rate_limit_burst_capacity: int = 50  # Burst capacity
    ...
    # Tenant Allow-list
    allowed_tenants: Optional[str] = None  # Comma-separated list of allowed tenant IDs
    ...
    def validate_default_top_k(cls, v):
        if v < 1 or v > 50:
            raise ValueError('Default top_k must be between 1 and 50')
        return v
    ...
    @validator('allowed_tenants')
    def validate_allowed_tenants(cls, v):
        if v is not None and v.strip():
            # Validate that all tenant IDs are at least 3 characters long
            tenant_ids = [tid.strip() for tid in v.split(',') if tid.strip()]
            for tenant_id in tenant_ids:
                if len(tenant_id) < 3:
                    raise ValueError(f'Tenant ID "{tenant_id}" must be at least 3 characters long')
        return v

    @validator('rate_limit_requests_per_second')
    def validate_rate_limit_requests_per_second(cls, v):
        if v <= 0 or v > 1000:
            raise ValueError('Rate limit requests per second must be between 0 and 1000')
        return v

    @validator('rate_limit_burst_capacity')
    def validate_rate_limit_burst_capacity(cls, v):
        if v < 1 or v > 1000:
            raise ValueError('Rate limit burst capacity must be between 1 and 1000')
        return v
    ...
    def allowed_tenant_list(self) -> list[str]:
        """Get list of allowed tenant IDs."""
        if not self.allowed_tenants or not self.allowed_tenants.strip():
            return []
        return [tid.strip() for tid in self.allowed_tenants.split(',') if tid.strip()]

    @property
    def has_tenant_allowlist(self) -> bool:
        """Check if tenant allow-list is configured."""
        return len(self.allowed_tenant_list) > 0
